  ...
  return (
    <section className="mt-6 rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
      <div className="flex items-start gap-3">
        <Paperclip className="mt-0.5 shrink-0 text-zinc-500" size={18} />
        <div className="flex-1">
          ...
          {uploadError && (
            <p className="mt-4 rounded-lg border border-red-900 bg-red-950/40 p-3 text-sm text-red-200">
              {uploadError}
            </p>
          )}
          ...
          {loading && (
            <p className="mt-4 text-sm text-zinc-400">Carregando anexos...</p>
          )}

          {!loading && attachments.length === 0 && (
            <p className="mt-4 rounded-xl border border-dashed border-zinc-700 bg-zinc-950/60 p-4 text-sm text-zinc-500">
              {canUpload
          ...
          {!loading && attachments.length > 0 && (
            <ul className="mt-4 space-y-2">
              {attachments.map((attachment) => (
                <li
                  key={attachment.id}
                  className="flex flex-col gap-3 rounded-xl border border-zinc-800 bg-zinc-950 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between"
                >
                  ...
                  <button
                    ...
                    className="flex shrink-0 items-center justify-center gap-2 rounded-lg border border-zinc-700 px-3 py-2 text-xs font-medium text-zinc-200 hover:bg-zinc-800 disabled:opacity-60"
                  >
