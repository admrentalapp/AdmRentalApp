...
const SECTIONS: Array<{
  ...
  { id: 'inicio', shortLabel: 'Início', label: 'Começar aqui', icon: Sparkles },
  { id: 'perfis', shortLabel: 'Perfis', label: 'Perfis de acesso', icon: Users },
  {
    id: 'cadastros',
    shortLabel: 'Cadastros',
    label: 'Cadastros iniciais',
    icon: Building2,
  },
  { id: 'fluxo', shortLabel: 'Fluxo', label: 'Fluxo do chamado', icon: Compass },
  {
    id: 'simulacao',
    shortLabel: 'Simulação',
    label: 'Simulação guiada',
    icon: PlayCircle,
  },
  {
    id: 'situacoes',
  ...
  { id: 'faq', shortLabel: 'FAQ', label: 'Dúvidas rápidas', icon: CircleHelp },
]
...
export function HelpPage({
  ...
  function goToSection(id: HelpSectionId) {
    setActiveSection(id)
  }

  function goPrevSection() {
    if (!canGoPrev) return
    goToSection(SECTIONS[sectionIndex - 1].id)
  }
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col lg:min-h-[calc(100svh-10rem)]">
      {/* Hero — compacto no mobile */}
      <section className="relative overflow-hidden rounded-2xl border border-border/80 bg-linear-to-br from-card via-card/85 to-background p-4 sm:rounded-3xl sm:p-7 lg:p-9">
        <div className="pointer-events-none absolute -right-16 -top-20 h-64 w-64 rounded-full bg-red-600/10 blur-3xl" />
        <div className="relative">
          <div className="flex items-center gap-2.5 sm:gap-3">
            <AppLogo className="h-8 w-auto max-w-[96px] object-contain sm:h-10 sm:max-w-[120px]" />
            <span className="inline-flex items-center gap-1.5 rounded-full border border-border/80 bg-background/60 px-2.5 py-1 text-[11px] font-medium text-muted-foreground sm:text-xs">
              <BookOpen size={12} className="text-red-600" />
              Help · v1.0
            </span>
          </div>
          <h3 className="mt-3 text-xl font-bold tracking-tight text-foreground sm:mt-5 sm:text-3xl lg:text-4xl">
            Aprenda o ADM Manutenção
            <span className="block text-red-600">de forma simples e visual</span>
          </h3>
          <p className="mt-2 hidden max-w-2xl text-sm leading-relaxed text-muted-foreground sm:mt-3 sm:block">
            Este manual vive dentro do sistema. No celular, navegue como um app:
            uma tela por vez, com abas e botões Anterior / Próximo.
          </p>
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] sm:mt-4 sm:flex-wrap [&::-webkit-scrollbar]:hidden">
            <QuickLink
              label="Clientes"
      ...
      {/* Chips horizontais — estilo app (sempre visíveis no mobile) */}
      <div
        ref={chipsRef}
        className="sticky top-0 z-20 -mx-5 mt-4 border-y border-border/60 bg-background/95 px-5 py-3 backdrop-blur-md sm:mx-0 sm:rounded-2xl sm:border sm:px-3 lg:hidden"
      >
          ...
          {SECTIONS.map((section) => {
            ...
            return (
              <button
                ...
                className={cn(
                  'inline-flex shrink-0 items-center gap-1.5 rounded-full px-3.5 py-2.5 text-xs font-semibold transition active:scale-[0.98]',
                  active
        ...
        <p className="mt-2 text-center text-[11px] text-muted-foreground">
          Tela {sectionIndex + 1} de {SECTIONS.length} · {currentMeta.label}
        </p>
      </div>

      <div className="mt-4 grid flex-1 gap-4 lg:mt-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:gap-6">
        {/* Sidebar desktop */}
        <nav className="hidden h-fit rounded-2xl border border-border bg-card p-3 lg:sticky lg:top-6 lg:block">
        ...
        {/* Conteúdo — uma seção por vez (app-like) */}
        <div
          ref={contentRef}
          className="min-w-0 space-y-5 pb-28 lg:pb-6"
        >
          {activeSection === 'inicio' && (
            <section className="space-y-4 sm:space-y-5">
              <SectionTitle
                ...
                subtitle="Um guia interativo com telas do programa, fluxos e simulações."
              />
              ...
              <HelpCallout tone="tip" title="Dica de ouro">
                ...
                usuários). Só depois opere chamados. Se pular essa ordem, o
                cliente pode não ver equipamentos ao abrir a OS.
              </HelpCallout>
            </section>
          )}
              ...
              <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                  ...
                  <p className="mt-3 text-sm text-muted-foreground">
                    O cliente abre com prioridade <strong>Média</strong>. O
                    gestor pode elevar para Alta ou Crítica na triagem.
                  </p>
                </div>
              </div>
            </section>
          ...
          {activeSection === 'cadastros' && (
            <section className="space-y-4 sm:space-y-5">
              <SectionTitle
                eyebrow="Implantação"
                title="Cadastros iniciais"
                subtitle="Siga esta sequência uma vez por empresa. Depois é só operar."
              />
              <div className="grid gap-4 xl:grid-cols-2 xl:gap-5">
                <MockCadastroFlow />
                <div className="space-y-0 rounded-2xl border border-border bg-card p-4 sm:p-5">
                  ...
                  <HelpStep number={2} title="Equipamentos">
                    ...
                    <strong>Alocar</strong> ao cliente/obra.
                  </HelpStep>
                  <HelpStep number={3} title="Estoque de peças">
                    Menu <strong>Estoque</strong> → Nova peça (SKU, nome,
                    mínimo). Use entradas/saídas no atendimento.
                  </HelpStep>
                  <HelpStep number={4} title="Usuários">
                    Menu <strong>Usuários</strong> → Novo usuário. Para Cliente,
                    vincule a empresa correta.
                  </HelpStep>
                </div>
              </div>
          ...
          {activeSection === 'fluxo' && (
            <section className="space-y-4 sm:space-y-5">
              ...
              {/* Fluxo vertical no mobile, horizontal no desktop */}
              <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                <div className="flex flex-col gap-2 sm:hidden">
                  {FLOW_STEPS.map((label, index) => (
                    <div key={label} className="flex items-center gap-3">
                      <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-red-600 text-[11px] font-bold text-white">
                        {index + 1}
                      </span>
                      <div className="flex-1 rounded-xl border border-border bg-background px-3 py-2.5 text-sm font-semibold text-foreground">
                        {label}
                      </div>
                    </div>
                  ))}
                ...
                <div className="hidden overflow-x-auto sm:block">
                  <div className="flex min-w-[640px] items-center gap-2">
                    {FLOW_STEPS.map((label, index, arr) => (
              ...
              <div className="overflow-hidden rounded-2xl border border-border bg-card">
                <div className="border-b border-border px-4 py-3 sm:px-5 sm:py-4">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    Os mesmos badges coloridos das telas do sistema.
                  </p>
                </div>
                <div className="divide-y divide-border">
                  {STATUS_GUIDE.map((row) => (
                    <div
                      key={row.status}
                      className="flex flex-col gap-2 px-4 py-3.5 sm:grid sm:grid-cols-[150px_1fr_160px] sm:items-center sm:gap-3 sm:px-5 sm:py-4"
                    >
          ...
          {activeSection === 'simulacao' && (
            <section className="space-y-4 sm:space-y-5">
              <SectionTitle
                eyebrow="Pratique"
                title="Simulação guiada"
                subtitle="Marina (cliente), Ana (gestora) e Carlos (técnico)."
              />
              ...
              <div className="flex gap-2 overflow-x-auto pb-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {simSteps.map((step, index) => (
                  <button
                    ...
                    className={cn(
                      'shrink-0 rounded-full px-3.5 py-2.5 text-xs font-semibold transition active:scale-[0.98]',
                      simStep === index
              ...
              <div className="rounded-2xl border border-border bg-card p-4 sm:p-5">
                ...
                <div className="mt-4 sm:mt-5">{simSteps[simStep].mock}</div>
                <div className="mt-4 flex gap-2 sm:mt-5">
                  <button
                    type="button"
                    disabled={simStep === 0}
                    onClick={() => setSimStep((s) => Math.max(0, s - 1))}
                    className="min-h-11 flex-1 rounded-xl border border-border px-3 text-sm font-medium text-foreground disabled:opacity-40 sm:flex-none sm:px-4"
                  >
                    Anterior
                  </button>
                  <button
                    ...
                    className="inline-flex min-h-11 flex-1 items-center justify-center gap-1 rounded-xl bg-red-600 px-3 text-sm font-semibold text-white disabled:opacity-40 sm:flex-none sm:px-4"
                  >
                    Próxima
          ...
          {activeSection === 'situacoes' && (
            <section className="space-y-4 sm:space-y-5">
              ...
              <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
                ...
                <SituationCard
                  title="Cancelamento"
                  text="Gestor define Cancelado quando a OS não deve seguir."
                />
                <SituationCard
                  title="Checklist"
          ...
          {activeSection === 'faq' && (
            <section className="space-y-4 sm:space-y-5">
              ...
              <p className="pb-2 text-center text-xs text-muted-foreground">
                ADM Rental Service · Help do ADM Manutenção
              </p>
            </section>
          )}

          {/* Navegação inferior desktop */}
          <div className="hidden items-center justify-between gap-3 border-t border-border pt-4 lg:flex">
            <button
              type="button"
              disabled={!canGoPrev}
              onClick={goPrevSection}
              className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2.5 text-sm font-medium text-foreground disabled:opacity-40"
            >
              <ArrowLeft size={16} />
              Anterior
            </button>
            ...
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl bg-red-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40"
            >
              Próximo
              <ArrowRight size={16} />
            </button>
      ...
      {/* Bottom bar mobile — estilo app */}
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-border bg-card/95 px-4 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md lg:hidden">
        <div className="mx-auto flex max-w-6xl items-center gap-2">
          <button
            type="button"
            disabled={!canGoPrev}
            onClick={goPrevSection}
            className="inline-flex min-h-12 min-w-12 items-center justify-center rounded-xl border border-border text-foreground disabled:opacity-35"
          ...
          <button
            ...
            disabled={!canGoNext}
            onClick={goNextSection}
            className="inline-flex min-h-12 flex-1 items-center justify-center gap-1.5 rounded-xl bg-red-600 px-4 text-sm font-semibold text-white disabled:opacity-35 sm:flex-none sm:px-5"
          >
            Próximo
            <ArrowRight size={16} />
          </button>
...
function SectionTitle({
  ...
  return (
    <div>
      <p className="text-[11px] font-semibold uppercase tracking-wide text-red-600 sm:text-xs">
        {eyebrow}
      </p>
...
function Faq({ q, a }: { q: string; a: string }) {
  return (
    <details className="group rounded-2xl border border-border bg-card px-4 py-3.5 sm:px-5 sm:py-4">
      <summary className="cursor-pointer list-none font-semibold text-foreground marker:content-none">
        <span className="flex items-center justify-between gap-3 text-sm sm:text-base">
          {q}
          <ChevronRight
            size={16}
...
function QuickLink({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex shrink-0 items-center gap-1 rounded-full border border-border bg-background/70 px-3 py-2 text-xs font-semibold text-foreground transition active:scale-[0.98] hover:border-red-500/40 hover:text-red-600"
    >
      {label}
      <ArrowRight size={12} />
    </button>
